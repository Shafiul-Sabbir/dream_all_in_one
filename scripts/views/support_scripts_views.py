...
def load_LoggedUser(request, company_id):
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        ...
        for user_data in all_user_data:
            ...
            if user_id:
                user_instance = User.objects.filter(old_id = user_id, company_id = company_instance).first()
            ...
            if not LoggedUser.objects.filter(old_id=fields['old_id'], company_id=company_instance).exists():
